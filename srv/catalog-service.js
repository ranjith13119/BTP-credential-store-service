...
module.exports = cds.service.impl(async function () {
    ...
    this.on('topSales', async (req) => {
        try {
            const tx = cds.tx(req);
            const results = await tx.run(`CALL "BTP_AP_CREDENTIALS_STORE_DB_SP_TopSales"(?,?)`, [req.data.amount]);
            return results;
        } catch (err) {
            console.error(err);
            return {};
    ...
    this.on('READ', SalesOrders, async (req) => {
        try {
            ...
            return await tx.send({
                query: req.query,
                headers: {
                    'Application-Interface-Key': await credStore.readCredentialValue('BTP_AP_CREDENTIALS_STORE', 'password', 'ApplicationInterfaceKey'),
                    'APIKey': await credStore.readCredentialValue('BTP_AP_CREDENTIALS_STORE', 'password', 'APIKeyHubSandbox')
}
            })
        } catch (err) {
            req.reject(err);
